...
# Defina o número de linhas e colunas para os gráficos
rows = 2  # Exemplo de quantidade de linhas
cols = 6  # Exemplo de quantidade de colunas

# Crie a figura para os gráficos
plt.figure(figsize=(20, 15))

# Gerar as curvas ROC para cada classe
for idx in enumerate(11):
    try:
        # Calcule a curva ROC
        ...
        plt.plot(fpr, tpr, color='blue', label=f'Classe {idx} (AUC = {roc_auc:.2f})')
        plt.plot([0, 1], [0, 1], color='grey', linestyle='--')  # Linha aleatória
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel('Falso Positivo')
        plt.ylabel('Verdadeiro Positivo')
        plt.title(f'ROC Classe {idx}')
        plt.legend(loc="lower right")
    except ValueError as e:
        print(f"Erro ao calcular curva ROC para classe {idx}: {e}")

# Ajustar o layout e salvar o gráfico
plt.tight_layout()
plt.savefig('ROC_classes.png')  # Corrigido para salvar o gráfico com um nome fixo
plt.show()
