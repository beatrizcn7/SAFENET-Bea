...
def calculate_metrics(cm):
    ...
    for i in range(len(cm)):
        ...
        TN = sum(cm.sum() for j in range(len(cm)) if j != i) - FP - FN  # Verdadeiros negativos
...
# Criar DataFrame para salvar as métricas
metrics_df = pd.DataFrame.from_dict(metrics, orient='index').reset_index()
metrics_df.rename(columns={'index': 'Class'}, inplace=True)

# Salvar as métricas em um arquivo Excel
metrics_df.to_excel('Métricas.xlsx', index=False)

print("Treinamento completo. Resultados salvos em 'Resultados 2.xlsx' e métricas em 'Métricas.xlsx'.")
