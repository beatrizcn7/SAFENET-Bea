...
def verificar_e_normalizar_xml(xml_path):
    try:
        ...
        if id_element is None or not id_element.text.isdigit() or int(id_element.text) not in valid_id_range:
            print(f'Id inválido no arquivo {xml_path}: {id_element.text if id_element is not None else "None"}')
            return False
        ...
        if intervalo_anos not in valid_intervalo_anos:
            print(f'Intervalo de Anos inválido no arquivo {xml_path}: {intervalo_anos}')
            return False
        ...
        if tipo_estrutura not in valid_tipo_estrutura:
            print(f'Tipo de Estrutura inválido no arquivo {xml_path}: {tipo_estrutura}')
            return False
        ...
        if material not in valid_material:
            print(f'Material inválido no arquivo {xml_path}: {material}')
            return False
